...
internal static partial class Program
{
    private static void RunGridCellSearch(VisualStyle style)
    {
        ...
        async Task Drive()
        {
            ...
            try
            {
                ...
                await Ui(() =>
                {
                    ...
                    inspector.Layout.Row.Text = "bad";
                    inspector.Layout.Column.Text = "99";
                    var selection = editor.Selection;
                    inspector.Layout.FindCell.Invoke();
                    Require(inspector.Layout.Row.Text == "0" && inspector.Layout.Column.Text == "1" &&
                    ...
                    inspector.Layout.FindCell.Focus();
                });
                await Ui(() =>
                {
                    var button = inspector.Layout.FindCell.GetBounds();
                    ...
                    Require(button.Width >= 100 && button.Y >= panel.Y && button.Y + button.Height <= panel.Y + panel.Height,
                        "The native helper button is visible through inspector scrolling.");
                    Select(workspace.Document!.Root!.Children[0].Children[0]);
                    inspector.Layout.FindCell.Invoke();
                    Require(inspector.Layout.Column.Text == "1" && inspector.Layout.Feedback.Text.Contains("parent Grid", StringComparison.Ordinal),
                        "A direct child finds space in its parent Grid.");
                    Select(workspace.Document.Root.Children[4].Children[0]);
                    inspector.Layout.FindCell.Invoke();
                    Require(inspector.Layout.Column.Text == "2" && inspector.Layout.Feedback.Text.Contains("selected Grid", StringComparison.Ordinal),
                        "A selected nested Grid takes precedence over its own parent.");
                    Select(workspace.Document.Root.Children[4]);
                    inspector.Layout.FindCell.Invoke();
                    Require(inspector.Layout.Column.Text == "1", "Selecting the outer Grid searches its own cells instead.");
                    inspector.Layout.Row.Text = "7";
                    inspector.Layout.Column.Text = "8";
                    ...
                    inspector.Layout.Insert.Invoke();
                    workspace.FindEmptyGridCell();
        ...
        void Refused(string reason) => Require(inspector.Layout.Row.Text == "7" && inspector.Layout.Column.Text == "8" &&

...
- Custom toolbar callbacks: `PostToolbarCommand` leaves the toolbar content scope before application commands create palette sources or change settings. `PostCustomization` defers settings changes from native input callbacks. `ExplorerCustomizationSmoke.cs` covers native button clicks, originating panes, availability checks, and resets.
- Palette shortcut rendering: `Drawing::shortcut_keycaps` supplies shared content-sized keycaps for styled and unstyled rows. `xui_collections_window_tests --palette-only` checks pixels with root typography, shortcut font overrides, aliases, sequences, and empty mappings.

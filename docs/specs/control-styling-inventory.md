...
| `NavigationList` | Collection parts, selection marker, icon color and size, badge, section header, disclosure indicator | selected, selected descendant, expanded, compact | `icon.size` sets the square image or glyph slot. Disabled ancestors and filtered items retain current navigation semantics. |

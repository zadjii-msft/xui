...
The declarative language includes these built-in nodes:
`VStack`, `HStack`, `Text`, `Button`, `Toggle`, `TextInput`, `Grid`, `DataGrid`, `NavigationView`, `ItemsView`, `ScrollView`, `Popup`, `SplitView`, `Reveal`, `RangeInput`, `Progress`, and `Content`.

...
enum class ImageMemoryFormat { png, jpeg };
struct ImageMemoryPlan {
    std::uint64_t generation{};
    ImageMemoryFormat format{};
    ImageSize source{}, output{}, hint{};
};
struct ImageMemoryState {
    std::uint64_t generation{};
    ImageStatus state{};
    ImageSize source{0, 0}, pixels{0, 0};
...
struct ImageLimits {
    ...
    static constexpr std::size_t encoded_bytes = 64 * 1024 * 1024;
...
struct ImageStatistics {
    ...
    std::size_t encoded_bytes{}, encoded_peak{};
};
...
public:
    ...
    void set_memory_source(std::span<const std::byte> encoded, ImageMemoryPlan plan);
    void cancel_memory();
    bool memory_backed() const { return memory_mode_; }
    ImageMemoryState memory_state() const;
    void on_memory_state(std::function<void()> callback);
    // Backend-only UI-thread handoff. Foreign observers must post metadata.
    bool sync_memory(const std::shared_ptr<TaskWake>& wake);
    const std::shared_ptr<const ImagePixels>& memory_pixels() const;
    void memory_uploaded();
    void memory_upload_failed();
protected:
...
private:
    ...
    std::uint64_t memory_generation_{};
    bool memory_mode_{};
    std::function<void()> memory_changed_;
};

...
void TabStrip::set_tabs(std::vector<TabItem> tabs, std::optional<std::uint64_t> selected) {
    for (std::size_t i = 0; i < tabs.size(); ++i) {
        if (tabs[i].icon < ButtonIcon::none || tabs[i].icon > ButtonIcon::chevron_down ||
...
Rect SplitView::divider() const {
    if (pane_area().width < 2 * minimum_pane_width + effective_divider_width() || progress_ == 0) return {};
...
void SplitView::set_ratio(float value) {
    ...
    const bool animate = transition_duration_ && !style_dragging_ && expanded() && progress_ == 1 &&
...
void SplitView::set_secondary_visible(bool value) {
    ...
    start_ = progress_;
    started_ = now;
    animating_ = transition_duration_ && progress_ != (value ? 1.0f : 0.0f);

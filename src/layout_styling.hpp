...
inline Rect aligned(Rect bounds, Size desired, const PartStyleValues* values) {
    ...
    const auto align = [](float& origin, float& extent, float desired, auto alignment) {
    ...
    align(bounds.x, bounds.width, desired.width, values->horizontal_alignment);
    align(bounds.y, bounds.height, desired.height, values->vertical_alignment);
    return bounds;
}

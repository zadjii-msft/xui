...
Point MapView::screen_point(GeoPoint point) const {
    ...
    return {static_cast<float>(b.width / 2 + dx * world), static_cast<float>(b.height / 2 + (p.y - c.y) * world)};
}
GeoPoint MapView::location_at(Point point) const {
    ...
    return unproject({c.x + (point.x - b.width / 2) / world, c.y + (point.y - b.height / 2) / world});
}
void MapView::pan(double x, double y) {
    valid(x); valid(y);
    const auto b = canvas_bounds();
    set_view(location_at({static_cast<float>(b.width / 2 - x), static_cast<float>(b.height / 2 - y)}), zoom_);
}
...
void MapView::cancel_request() { stop_.request_stop(); ++generation_; }
...
bool MapView::complete(const MapRequest& request, MapOverlay overlay, std::wstring error) {
    ...
    else { cancel_request(); error_ = std::move(error); invalidate(Invalidation::paint); }
    return true;
}
void MapView::arrange(Rect b) {
    const auto before = canvas_bounds();
    VectorCanvas::arrange(b);
    const auto after = canvas_bounds();
    if (before.width != after.width || before.height != after.height) rebuild();
}
void MapView::rebuild() {
    std::vector<VectorShape> shapes;
    ShapeId id = (1ull << 63);
    const auto clip = canvas_bounds();
    const double world = 256 * std::exp2(zoom_);
    for (int lon = -180; lon < 180; lon += 30) {
        const auto p = screen_point({0, static_cast<double>(lon)});
        VectorShape s; s.id = id++; s.points = {{p.x, 0}, {p.x, clip.height}}; s.stroke = {0.3f, 0.4f, 0.5f, 1}; s.clip = clip; shapes.push_back(s);
    }
    for (int lat = -60; lat <= 60; lat += 30) {
        const auto p = screen_point({static_cast<double>(lat), center_.longitude});
    ...
    for (const auto& line : overlay_.lines) {
        for (std::size_t i = 1; i < line.points.size(); ++i) {
            auto a = screen_point(line.points[i - 1]), b = screen_point(line.points[i]);
    ...
    for (const auto& marker : overlay_.markers) {
        auto p = screen_point(marker.location);

...
CaptionHit TitleBar::hit_test(Point point) const {
    ...
    if (secondary_tabs_->visible() && inside(secondary_tabs_->bounds()) &&
        secondary_tabs_->hit_test(Point{point.x - secondary_tabs_->bounds().x, point.y - secondary_tabs_->bounds().y})) return CaptionHit::client;
    if (inside(tabs_->bounds()) && tabs_->hit_test(Point{point.x - tabs_->bounds().x, point.y - tabs_->bounds().y})) return CaptionHit::client;
